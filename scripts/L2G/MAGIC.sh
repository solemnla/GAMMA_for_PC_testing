...
#  MAGIC analysis
# ------------------------------------------------------------------------
magic_functions_file=$(eval echo $(yq .software.magic "${CONFIG}"))
gencode_file=$(eval echo $(yq .gene.gencode "${CONFIG}"))

CpG_link_file=$(eval echo $(yq .magic.CpG_link "${CONFIG}"))
hQTL_link_file=$(eval echo $(yq .magic.hQTL_link "${CONFIG}"))
caQTL_link_file=$(eval echo $(yq .magic.caQTL_link "${CONFIG}"))

reference_bim_file=$(eval echo $(yq .reference.reference_all_bim "${CONFIG}"))
QTL_name_list_file=$(eval echo $(yq .magic.QTL_name_list "${CONFIG}"))
